...
export const useRejectGuestCar = (
  options?: MutationOptions<void, TServerError, TRejectCarPayload>
) =>
  useMutation<void, TServerError, TRejectCarPayload>({
    mutationFn: ({ id, reason }) => carRegisterService.rejectGuestCar(id, reason),
    onError: (e) => notifyError(e, 'Từ chối đăng ký xe khách thất bại'),
    ...options
  })
