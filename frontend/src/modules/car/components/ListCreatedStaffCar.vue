...
import { useGetCreatedStaffCarList } from '../composables/queries/carList/useGetCreatedStaffCarList'
...
const columns: (Omit<ColumnProps, 'field'> & {
  field?: string | ((item: RequestAndCreatedStaffCarVM) => string)
  filterSlot?: string
  customSlot?: string
  ...
  },
  {
    header: 'Ngày tạo đơn',
    field: (data) =>
      data?.createdAt ? DateTime.fromISO(data?.createdAt).toFormat('dd/MM/yyyy') : '--'
  },
  {
    field: 'status',
    header: 'Trạng thái',
...
const { data, isLoading } = useGetCreatedStaffCarList(
  () =>
    cleanObject({
...
const handleRowClick = (event: DataTableRowClickEvent<RequestAndCreatedStaffCarVM>) => {
  modalDetailCarModalRef?.value?.openModal(event.data.id, 'STAFF')
}

const handleAfterProcessStaffCar = () => {
  queryClient.invalidateQueries({ queryKey: ['getRequestStaffCarList'] })
}
