...
import { useGetStudentCarList } from '../composables/queries/carList/useGetStudentCarList'
...
const columns = [
  ...
  {
    header: 'Họ tên người học',
    field: 'studentName'
  },
  {
    header: 'Mã số người học',
    field: 'studentCode'
  },
  {
    header: 'Khoá',
  ...
  {
    header: 'Thời điểm tốt nghiệp',
    field: 'graduationDate',
    customSlot: 'graduationDate'
  },
  {
    header: 'Biển số xe',
    field: 'licensePlate'
...
const { data, isLoading } = useGetStudentCarList(
  () =>
    cleanObject({
...
const { mutate: deleteStudentCar, isPending: isDeletingCar } = useDeleteCar({
  onSuccess: () => {
    toastSucceed({ detail: 'Xoá đăng ký xe thành công' })
    queryClient.invalidateQueries({ queryKey: ['getStudentCarList'] })
  }
})

const confirmDeleteStudentCar = (car: ListStudentCarVM) => {
  confirm.require({
    group: 'confirmDeleteInListStudentCar',
    message: `Sau khi xóa, xe sẽ không thể ra vào Đại học. Thầy/Cô có chắc chắn muốn xóa?`,
    header: 'Xóa đăng ký xe',
    data: car,
    accept: () => {
      deleteStudentCar(car?.id)
    },
    rejectProps: {
      label: 'Hủy',
      severity: 'secondary',
...
const handleRowClick = (event: DataTableRowClickEvent<ListStudentCarVM>) => {
  modalDetailCarModalRef?.value?.openModal(event.data.id, 'STUDENT')
}
</script>
...
<template>
  ...
  >
    ...
    <template #graduationDate="{ data }">
      <span>{{
        `${data?.graduationDate ? DateTime.fromISO(data?.graduationDate).toFormat('dd/MM/yyyy') : '--'}`
      }}</span>
    </template>
    <template #tableAction="{ data }">
      <div class="flex items-center justify-end gap-2" v-if="data.canDelete">
        <Button
          variant="contained"
          @click="() => confirmDeleteStudentCar(data)"
          severity="danger"
  ...
  <ConfirmDialog group="confirmDeleteInListStudentCar" class="w-[300px]">
    <template #message="{ message }">
      <div>
        ...
        <div>
          ...
          >{{ (message?.data as ListStudentCarVM)?.licensePlate }}
        </div>
      </div>
    </template>
  </ConfirmDialog>
