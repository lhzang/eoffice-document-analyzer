...
import { useGrantUnitCarPermission } from '../composables/queries/carDelagation/useGrantUnitCarPermission'
import { useRevokeUnitCarPermission } from '../composables/queries/carDelagation/useRevokeUnitCarPermission'
...
const { mutate: updatePermissionConfig, isPending: isUpdatingConfig } = useGrantUnitCarPermission({
  onSuccess: () => {
    toastSucceed({ detail: 'Phân quyền duyệt và đăng ký thành công' })
    emit('updatedConfig')
  }
})
const { mutate: revokePermission, isPending: isRevokingPermission } = useRevokeUnitCarPermission({
  onSuccess: () => {
  ...
  >
    <div class="relative h-full p-4">
      ...
      <div class="mt-6 flex h-[300px] w-full flex-col gap-2 overflow-auto">
        <template v-if="filterDisplayCurrentPermissionStaffs?.length">
          <div
