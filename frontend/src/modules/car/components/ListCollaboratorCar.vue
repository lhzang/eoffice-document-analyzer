...
import { useGetCollaboratorCarList } from '../composables/queries/carList/useGetCollaboratorCarList'
...
const { data, isLoading } = useGetCollaboratorCarList(
  () =>
    cleanObject({
...
const { mutate: deleteCollaboratorCar, isPending: isDeletingCar } = useDeleteCar({
  onSuccess: () => {
    toastSucceed({ detail: 'Xoá đăng ký xe thành công' })
    queryClient.invalidateQueries({ queryKey: ['getCollaboratorCarList'] })
  }
})

const confirmDeleteCollaboratorCar = (car: ListCollaboratorCarVM) => {
  confirm.require({
    group: 'confirmDeleteInListCollabCar',
    message: `Sau khi xóa, xe sẽ không thể ra vào Đại học. Thầy/Cô có chắc chắn muốn xóa?`,
    header: 'Xóa đăng ký xe',
    data: car,
    accept: () => {
      deleteCollaboratorCar(car?.id)
    },
    rejectProps: {
      label: 'Hủy',
      severity: 'secondary',
...
const handleRowClick = (event: DataTableRowClickEvent<ListCollaboratorCarVM>) => {
  modalDetailCarModalRef?.value?.openModal(event.data.id, 'COLLABORATOR')
}
</script>
...
<template>
  ...
  >
    ...
    <template #tableAction="{ data }">
      <div class="flex items-center justify-end gap-2" v-if="data.canDelete">
        <Button
          variant="contained"
          @click="() => confirmDeleteCollaboratorCar(data)"
          severity="danger"
  ...
  <ConfirmDialog group="confirmDeleteInListCollabCar" class="w-[300px]">
    <template #message="{ message }">
      <div>
        ...
        <div>
          ...
          >{{ (message?.data as ListCollaboratorCarVM)?.licensePlate }}
        </div>
      </div>
    </template>
  </ConfirmDialog>
