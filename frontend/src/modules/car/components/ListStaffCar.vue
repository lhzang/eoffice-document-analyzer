...
import { useGetStaffCarList } from '../composables/queries/carList/useGetStaffCarList'
...
const { data: staffListData, isLoading } = useGetStaffCarList(
  () =>
    cleanObject({
...
const { mutate: deleteStaffCar, isPending: isDeletingCar } = useDeleteCar({
  onSuccess: () => {
    toastSucceed({ detail: 'Xoá đăng ký xe thành công' })
    queryClient.invalidateQueries({ queryKey: ['getStaffCarList'] })
  }
})

const confirmDeleteStaffCar = (car: ListStaffCarVM) => {
  confirm.require({
    group: 'confirmDeleteInListStaffCar',
    message: `Sau khi xóa, xe sẽ không thể ra vào Đại học. Thầy/Cô có chắc chắn muốn xóa?`,
    header: 'Xóa đăng ký xe',
    data: car,
    accept: () => {
      deleteStaffCar(car?.id)
    },
    rejectProps: {
      label: 'Hủy',
      severity: 'secondary',
...
const handleRowClick = (event: DataTableRowClickEvent<ListStaffCarVM>) => {
  modalDetailCarModalRef?.value?.openModal(event.data.id, 'STAFF')
}

const handleHandleAfterEditETagSuccess = () => {
  modalDetailCarModalRef?.value?.closeModal()
  queryClient.invalidateQueries({ queryKey: ['getStaffCarList'] })
}
</script>
...
<template>
  <AppTable
    :data="staffListData?.docs ?? []"
    :columns="columns"
    :loading="isLoading"
    paginator
    :always-show-paginator="true"
    :totalRecords="staffListData?.docCount"
    :lazy="true"
  ...
  >
    <template #order="{ index }">
      <span>{{
        (staffListData?.page ?? 0) * (staffListData?.pageSize ?? tablePagination.pageSize) +
        index +
        1
      }}</span>
    </template>
    <template #tableAction="{ data }">
      <div class="flex items-center justify-end gap-2" v-if="data?.canDelete">
        <Button
          variant="contained"
          @click="() => confirmDeleteStaffCar(data)"
          severity="danger"
  ...
  <ConfirmDialog group="confirmDeleteInListStaffCar" class="w-[300px]">
    <template #message="{ message }">
      <div>
        <div class="mb-4 text-lg font-semibold text-red-500">{{ message?.message }}</div>

        <div>
          <span class="text-primary font-semibold">Tên chủ xe : </span
          >{{ (message?.data as ListStaffCarVM)?.name }}
        </div>
        <div>
          <span class="text-primary font-semibold">Biển số xe : </span
          >{{ (message?.data as ListStaffCarVM)?.licensePlate }}
        </div>
      </div>
    </template>
  </ConfirmDialog>
  <ModalDetailCar
    ...
    @update-etag-success="handleHandleAfterEditETagSuccess"
  />
</template>
