...
import { useGetRejectedODList } from '../composables/queries/useGetRejectedODList'
...
const { data, isLoading, refetch } = useGetRejectedODList(() => ({
  ...props.filterParams,
...
<template>
  ...
  >
    ...
    <template #actionSlot="{ data }">
      <Button @click.stop="router.push(`/out-doc/re-register/${data?.id}`)">Trình lại</Button>
    </template>
  </AppTable>
