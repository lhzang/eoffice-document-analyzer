...
//props
const { readOnlySelectedDestinations = new Map<string, TFormSelectDestinationValue>() } =
  defineProps<TProps>()
...
watchEffect(() => {
  console.log(
    listExternalUnitsData?.value?.map((unit) => ({
    ...
    >
      ...
      <div class="flex w-10 shrink-0 items-center justify-center">
        <Checkbox
          ...
          :model-value="
            readOnlySelectedDestinations?.get(unit?.id)?.formName ||
            modelValue?.get(unit.id)?.formName
              ? [selectedDestinations?.get(unit.id) ?? readOnlySelectedDestinations?.get(unit?.id)]
              : null
          "
          :disabled="readOnlySelectedDestinations?.has(unit?.id)"
          @update:modelValue="
