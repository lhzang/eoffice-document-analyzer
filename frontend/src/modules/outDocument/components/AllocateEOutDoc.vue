...
import { useIssueEOutDoc } from '../composables/queries/useIssueEOutDoc.ts'
import { allocateEOutDocSchema } from '../schemas/documentSchema'
...
const { mutate: issueEOutDoc } = useIssueEOutDoc({
  onSuccess: () => {
    toastSucceed({
...
const schema = allocateEOutDocSchema.extend({
  outOrdinal: allocateEOutDocSchema.shape.outOrdinal.superRefine((value, ctx) => {
