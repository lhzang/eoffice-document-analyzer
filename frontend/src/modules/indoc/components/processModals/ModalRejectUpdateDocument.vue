...
const { mutate: rejectXroadID, isPending } = useSkipXroad({
  onSuccess: () => {
    toastSucceed({
      detail: 'Từ chối văn bản thành công'
    })
    emit('rejectSuccess')
...
const confirmReturn = (values: FinishFormValues) => {
  confirm.require({
    group: 'return',
    message: `Thầy/Cô chắc chắn muốn Từ chối văn bản?`,
    header: 'Từ chối',
    accept: () => {
      rejectXroadID({
        id: documentId,
...
<template>
  <AppModal title="Từ chối văn bản" v-model:visible="isVisible" :wrapper-style="{ width: '480px' }">
    <form @submit="onSubmit">
      <AppTextarea
        ...
        :limitNumber="500"
        :placeholder="`Nhập lý do Từ chối văn bản (nếu có)`"
      />
      <!-- <AppFileInput
        name="files"
        :multiple="true"
