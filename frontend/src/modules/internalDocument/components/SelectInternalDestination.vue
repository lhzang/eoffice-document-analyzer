...
const { data: allUnitTrees, isLoading: isGettingUnitsData } = useGetDestination()
...
const checkIfTickedAll = (formInput: TDocumentProcessRole) => {
  ...
  return allEnableRootUnits?.value?.every((availableRoot) => {
    ...
    else return selectedDestinations?.value?.has(availableRoot?.id)
  })
}
...
<template>
  <div>
    ...
    <div
      class="border-shadow"
      @click="console.log(readOnlySelectedDestinations, 'readOnlySelectedDestinations')"
    >
      ...
      <div class="relative">
        ...
        <div v-else class="relative h-[400px] min-h-[200px] overflow-auto">
          ...
          <template v-for="(unitTree, index) in allUnitTrees" :key="index">
