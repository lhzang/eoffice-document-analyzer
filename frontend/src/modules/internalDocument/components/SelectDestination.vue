...
const tempValue = ref<TFormSelectDestinationValues>(
  new Map(defaultValue) ?? new Map(selectedDestinations?.value) ?? new Map()
)
...
watch(
  ...
  (newVal) => {
    if (
      ...
      selectedDestinations.value = new Map(newVal)
      initialized = true
    }
  },
  { immediate: true }
...
const handleOpenModalSelectDestination = () => {
  ...
  tempValue.value = new Map(selectedDestinations.value) ?? new Map()
...
const handleConfirmSelect = () => {
  ...
  selectedDestinations.value = new Map(tempValue.value)
  emit('submit', selectedDestinations.value)
  modalRef?.value?.closeModal()
}

const handleCancel = () => {
  tempValue.value = new Map(defaultValue) ?? new Map()
}
</script>
...
<template>
  ...
  <AppModal :title="modalLabel" ref="modalRef" :wrapper-style="{ width: '60%' }">
    ...
    <div>
      <!-- <SelectExternalDestination v-if="selectedTab === 'EXTERNAL_TAB'" v-model="tempValue" /> -->
      <SelectInternalDestination
        v-if="selectedTab === 'INTERNAL_TAB'"
        :readOnlySelectedDestinations
        v-model="tempValue"
      />
    </div>
