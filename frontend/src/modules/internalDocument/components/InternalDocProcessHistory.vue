...
import { useGetInternalDocProcessHistory } from '../composables/queries/useGetInternalDocProcessHistory'
...
const {
  data: processHistory,
  isError: isGetProcessHistoryError,
  isLoading: isGettingProcessHistory
} = useGetInternalDocProcessHistory(() => documentId)
</script>
<template>
  <div
  ...
  <div
    v-else-if="isGetProcessHistoryError"
    class="flex h-full w-full items-center justify-center px-4 py-2"
  >
                ...
                <div v-if="staff?.status && staff?.processTime">
                    ...
                    <div
                      v-tippy="
                        staff?.status === OUT_DOC_PROCESS_STEP_VALUES.Signed
                          ? getDisplayTypeLabel(staff?.signType, true)
                          : ''
                      "
                    >
                  ...
                  >
                    ...
                    <div class="max-h-[200px] w-full truncate overflow-auto">
                      ...
                      />
                      <ModalPreviewFile v-if="staff?.files?.length" ref="previewModalRef" />
                    </div>
                  </div>
                </div>
              </div>
