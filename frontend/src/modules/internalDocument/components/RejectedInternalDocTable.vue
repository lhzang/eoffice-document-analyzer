...
import { useGetRejectedInternalDocList } from '../composables/queries/useGetRejectedInternalDocList'
...
const modalDetailInternalDocRef = ref<TModalDetailInternalDocRef | null>(null)
...
const { data, isLoading, refetch } = useGetRejectedInternalDocList(() => ({
  ...props.filterParams,
...
<template>
  ...
  >
    ...
    <template #actionSlot="{ data }">
      <Button @click.stop="router.push(`/internal-doc/re-register/${data?.id}`)">Trình lại</Button>
    </template>
  </AppTable>
