...
import { genMockAdminDocTypeData, internalData } from './mockData'
...
export const handlers = [
  http.get(
    `${appConfig.VITE_API_SERVER}/api/task/internal-documents`,
    withAuth(async ({ request }) => {
      ...
      if (Object.keys(filters).length === 0) {
        ...
        newData = filterDocuments(internalData, filters)
      }
