...
import FullTreeSelectSigner from './FullTreeSelectSigner.vue'
...
type TProps = {
  ...
  filterPermision?: TAppFeatureKey
...
const displayValues = computed(() => {
  console.log(internalStaffs, 'internalStaffs')
...
const handleConfirmSelect = () => {
  if (!(tempUnitOnlySelect.value as TStaffSelectValue[])?.length && required) {
    return toastWarning({ detail: 'Vui lòng chọn ít nhất một cá nhân' })
  }
    ...
    <div :class="`h-full w-full`" v-bind="$attrs">
      ...
      <AppModal :title="modalLabel" ref="modalRef" :wrapper-style="{ width: '60%' }">
        <div class="grid grid-cols-8 gap-4">
          <FullTreeSelectSigner
            class="border-shadow col-span-5"
            :model-value="tempUnitOnlySelect"
