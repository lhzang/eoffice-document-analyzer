...
import InternalUnitSignerSelect from './InternalUnitSignerSelect.vue'
...
const selectedStaffs = defineModel<(TIsMultiple extends false ? TSigner : TSigner[]) | null>({
  required: true
})
...
const { data: unitForest, isLoading: isGettingUnitsData } = useGetAllInternalUnitWithStaff(
  ...
  { structuralSharing: true, gcTime: 0, staleTime: 0, refetchOnMount: 'always' }
)
...
const handleFetchStaffInUnit = (unit: TTreeUnitWithStaffNode) => {
  if (
    ...
    triggerFetchStaff(
      { unitId: unit?.id, permissionFilter: props?.permissionFilter },
      {
        onSuccess: (staffList) => {
...
onUnmounted(() => {
  queryClient.cancelQueries({ queryKey: ['getAllTreeInternalWithStaff'] })
})
</script>
<template>
  <div>
