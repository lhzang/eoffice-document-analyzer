...
import { useGetDecendantInternalUnitWithStaff } from '@/shared/composables/queries/organization/unit/useGetDecendantInternalUnitWithStaff'
...
const successFetchedUnits = new Set()

const { data: unitForest, isLoading: isGettingUnitsData } = useGetDecendantInternalUnitWithStaff(
  () => props.rootUnitId,
...
const handleFetchStaffInUnit = (unit: TTreeUnitWithStaffNode) => {
  if (
    unit?.isGroup ||
    successFetchedUnits.has(`${unit?.id}_${unit.rootNodeId}`) ||
    ...
    triggerFetchStaff(
      ...
      {
        onSuccess: (staffList) => {
          successFetchedUnits?.add(`${unit?.id}_${unit.rootNodeId}`)
          if (unitForest?.value) {
            queryClient.setQueryData(
              ['getDecendantTreeInternalWithStaff', props.rootUnitId],
